lci = 0.000014; // quinas inferiores, 14% de rv
lcs = 0.000014; // quinas superiores, 14% de rv
le  = 0.000007; // elipse, 7% de rv

rh = 0.00005; // raio horizontal
rv = 0.00010; // raio vertical
Dh = 3*rh;
kh = 2*rh + rv; // permeabilidade horizontal
kv = 2*rv + rh; // permeabilidade vertical
xc = 3.0*rh; // x centro da 1a. elipse
yc = 3.0*rh; // y centro da 1a. elipse

nh = 2; // total de obstaculos horizontais
nv = 4; // total de obstaculos verticais

Lx = 2*xc + (nh-1)*kh; // comprimento da malha
Ly = yc + (nv-1)*kv + Dh; // largura da malha
//+
Point(1) = {         0,  0, 0, lci};
Point(2) = {        Lx,  0, 0, lci};
Point(3) = {        Lx, Ly, 0, lcs};
Point(4) = {  Lx-xc+rh, Ly, 0, le };
Point(5) = {     xc-rh, Ly, 0, le };
Point(6) = {         0, Ly, 0, lcs};
//+
Line(1) = {1, 2};
Line(2) = {2, 3};
