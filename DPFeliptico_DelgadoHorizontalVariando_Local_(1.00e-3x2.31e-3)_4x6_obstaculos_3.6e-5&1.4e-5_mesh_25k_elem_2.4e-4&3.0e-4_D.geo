lci = 0.000036; // quinas inferiores, 3x 10% de rh2
lcs = 0.000036; // quinas superiores, 3x 10% de rh2
le  = 0.000014; // elipse, 17% de rh2

rh1 = 0.00015; // raio horizontal 1
rh2 = 0.00012; // raio horizontal 2
rv = 0.00005; // raio vertical
Dv = 2*rv;
kh = 3*rh1; // permeabilidade horizontal
kv = 3*rv; // permeabilidade vertical
xc = 4*rh2; // x centro da 1a. elipse
yc = 3*rv; // y centro da 1a. elipse

nh = 4; // total de obstaculos horizontais
nv = 6; // total de obstaculos verticais
...
Point(2) = {        Lx,  0, 0, lci};
Point(3) = {        Lx, Ly, 0, lcs};
Point(4) = {Lx-xc+rh1, Ly, 0, le };
Point(5) = {   xc-rh1, Ly, 0, le };
Point(6) = {         0, Ly, 0, lcs};
//+
Line(1) = {1, 2};
Line(2) = {2, 3};
...
For h In {0:nh-1}
 For v In {0:nv-1}
  If (v % 2 == 0)
    ...
    Point(8+count)  = {xc+rh1+kh*(h),    yc+kv*(v), 0, le};  // direita da elipse
    Point(9+count)  = {   xc+kh*(h), yc+rv+kv*(v), 0, le};  // cima da elipse
    Point(10+count) = {xc-rh1+kh*(h),    yc+kv*(v), 0, le};  // esquerda da elipse
    Point(11+count) = {   xc+kh*(h), yc-rv+kv*(v), 0, le};  // baixo da elipse
  Else
    //+
    Point(7+count)  = {   xc+kh*(h),    yc+kv*(v), 0, le};  // centro da elipse
    Point(8+count)  = {xc+rh2+kh*(h),    yc+kv*(v), 0, le};  // direita da elipse
    Point(9+count)  = {   xc+kh*(h), yc+rv+kv*(v), 0, le};  // cima da elipse
    Point(10+count) = {xc-rh2+kh*(h),    yc+kv*(v), 0, le};  // esquerda da elipse
    Point(11+count) = {   xc+kh*(h), yc-rv+kv*(v), 0, le};  // baixo da elipse
  EndIf
  //+
