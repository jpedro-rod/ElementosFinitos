lci = 0.001; // quinas inferiores
lcs = 0.001; // quinas superiores
le  = 0.0001; // elipse

rh = 0.0005; // raio horizontal
rv = 0.0005; // raio vertical
Dh = 2*rh; Dv = 2*rv;
xc = 3*Dh + rh; // x centro da 1a. elipse
yc = 3*Dv + rv; // y centro da 1a. elipse
kh = 0.0015; // permeabilidade horizontal
kv = 0.0015; // permeabilidade vertical

nh = 6; // total de obstaculos horizontais
nv = 5; // total de obstaculos verticais

Lx = 2*xc + (nh-1)*kh; // comprimento da malha
Ly = yc + (nv-1)*kv + Dv+rv; // largura da malha
//+
Point(1) = {         0,  0, 0, lci};
Point(2) = {        Lx,  0, 0, lci};
Point(3) = {        Lx, Ly, 0, lcs};
Point(4) = {Lx-xc+2*rh, Ly, 0, le };
Point(5) = {   xc-2*rh, Ly, 0, le };
Point(6) = {         0, Ly, 0, lcs};
//+
Line(1) = {1, 2};
Line(2) = {2, 3};
Line(3) = {3, 4};
Line(4) = {4, 5};
Line(5) = {5, 6};
Line(6) = {6, 1};

Curve Loop(1) = {3, 4, 5, 6, 1, 2};

// elipses horizontais (nh) e verticais (nv)
count=0;counte=0;countl=2;
For v In {0:nv-1}
 For h In {0:nh-1}
  //+
  Point(7+count)  = {   xc+kh*(h),    yc+kv*(v), 0, le};  // centro da elipse
  Point(8+count)  = {xc+rh+kh*(h),    yc+kv*(v), 0, le};  // direita da elipse
  Point(9+count)  = {   xc+kh*(h), yc+rv+kv*(v), 0, le};  // cima da elipse
  Point(10+count) = {xc-rh+kh*(h),    yc+kv*(v), 0, le};  // esquerda da elipse
  Point(11+count) = {   xc+kh*(h), yc-rv+kv*(v), 0, le};  // baixo da elipse

  //+
  Ellipse(7+(counte))  = {9+count, 7+count, 7+count, 8+count}; // Faz arco eliptico com dois pontos e um centro
  Ellipse(8+(counte))  = {8+count, 7+count, 7+count, 11+count};
  Ellipse(9+(counte))  = {11+count, 7+count, 7+count, 10+count};
  Ellipse(10+(counte)) = {10+count, 7+count, 7+count, 9+count};

  Curve Loop(countl) = {10+counte, 7+counte, 8+counte, 9+counte};
  count+=5;counte+=4;countl+=1;
 EndFor
EndFor

Plane Surface(1) = {1:nh*nv+1};

//+
Physical Line("inferior", 57)  = {1};
Physical Line("superior", 58)  = {3,4,5};
Physical Line("esquerda", 59)  = {6};
Physical Line("direita", 60)   = {2};
Physical Line("obstaculo1",61) = {7+0*nh*4:1*nh*4+6};
Physical Line("obstaculo2",62) = {7+1*nh*4:2*nh*4+6};
Physical Line("obstaculo3",63) = {7+2*nh*4:3*nh*4+6};
Physical Line("obstaculo4",64) = {7+3*nh*4:4*nh*4+6};
Physical Line("obstaculo5",65) = {7+4*nh*4:5*nh*4+6};
